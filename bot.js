...
// const BOT_TOKEN = '8397810122:AAHOtH0daLYtae1bQSZqHoum11OZUFfniGc';
...
bot.on('photo', async (ctx) => {
    ...
    try {
        ...
        const fileUrl = `https://api.telegram.org/file/bot${BOT_TOKEN}/${file.file_path}`; // 🔴 Исправлено: убрана лишняя пробел
...
bot.on('text', async (ctx) => {
    ...
    ctx.reply(`✅ Текст принят: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`, mainKeyboard);
...
async function createAndSendVideo(ctx, userId) {
    try {
        ...
        try {
            ...
            // Создаем видео с аудио (15 секунд)
            ctx.reply('🎬 Создаю 15-секундное видео с озвучкой...');
            outputPath = path.join(OUTPUT_DIR, `video_${uuidv4()}.mp4`);
            await createVideo(user.images, audioPath, user.text, outputPath);
            
            // Не удаляем аудио файл — закомментировано по требованию
            // setTimeout(async () => {
            //     try {
            //         await fs.remove(audioPath);
            //         console.log(`Аудио файл удалён: ${audioPath}`);
            //     } catch (error) {
        ...
        await ctx.replyWithVideo({ source: outputPath }, { 
            caption: '🎬 Ваше видео готово! (15 секунд)',
...
//             // Создаем видео с аудио (должно быть 15 секунд)
//             ctx.reply('🎬 Создаю 15-секундное видео с озвучкой...');
//             outputPath = path.join(OUTPUT_DIR, `video_${uuidv4()}.mp4`);
//             await createVideo(user.images, audioPath, user.text, outputPath, 15); // ← Передаём длительность
...
//             // Fallback: видео без звука (на 15 секунд)
//             ctx.reply('⚠️ Не удалось создать озвучку. Создаю видео без звука...');
//             outputPath = path.join(OUTPUT_DIR, `video_simple_${uuidv4()}.mp4`);
//             await createSimpleVideo(user.images, user.text, outputPath, 15); // ← Передаём длительность
//         }
        
//         // Отправляем видео
//         ctx.reply('📤 Отправляю видео...');
//         await ctx.replyWithVideo({ source: outputPath }, { 
//             caption: '🎬 Ваше видео готово! (15 секунд с озвучкой)',
//             reply_markup: mainKeyboard.reply_markup 
//         });
