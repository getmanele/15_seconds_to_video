...
// /**
//  * Создание видео с изображениями и аудио
//  */
// async function createVideo(imagePaths, audioPath, text, outputPath) {
//     return new Promise((resolve, reject) => {
//         console.log('🎬 Начинаю создание видео...');
...
//             console.log(`Добавляю изображение ${index + 1}: ${imagePath}`);
...
//         // Создаем фильтр для слайдшоу
//         let filterComplex = '';
        
//         if (imagePaths.length === 1) {
//             // Одно изображение - показываем его на протяжении всего видео
//             filterComplex = `[0:v]scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,drawtext=text='${text}':fontsize=48:fontcolor=white:x=(w-text_w)/2:y=h-th-10:shadowcolor=black:shadowx=2:shadowy=2[v0]`;
//         } else {
//             // Несколько изображений - создаем слайдшоу
//             const durationPerImage = 15 / imagePaths.length; // Распределяем 15 секунд между изображениями
            
//             imagePaths.forEach((_, index) => {
//                 if (index === 0) {
//                     filterComplex += `[${index}:v]scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,drawtext=text='${text}':fontsize=48:fontcolor=white:x=(w-text_w)/2:y=h-th-10:shadowcolor=black:shadowx=2:shadowy=2[v${index}]`;
//                 } else {
//                     filterComplex += `;[${index}:v]scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,drawtext=text='${text}':fontsize=48:fontcolor=white:x=(w-text_w)/2:y=h-th-10:shadowcolor=black:shadowx=2:shadowy=2[v${index}]`;
//                 }
//             });
...
//             filterComplex += `;${concatFilter}concat=n=${imagePaths.length}:v=1:a=0[outv]`;
//         }
        
//         // Настройки команды
//         command
//             .complexFilter(filterComplex, imagePaths.length === 1 ? 'v0' : 'outv')
//             .outputOptions([
//                 '-c:v libx264',
//                 '-preset fast',
//                 '-crf 23',
...
// /**
//  * Создание простого видео без аудио (fallback)
//  */
// async function createSimpleVideo(imagePaths, text, outputPath) {
//     return new Promise((resolve, reject) => {
//         console.log('🎬 Создаю простое видео без аудио...');
...
//         command = command.input(imagePaths[0]);
...
//                 {
//                     filter: 'scale',
//                     options: '1280:720:force_original_aspect_ratio=decrease'
//                 },
//                 {
//                     filter: 'pad',
...
//             })
//             .on('error', (err) => {
//                 console.error('❌ Ошибка создания простого видео:', err);
//                 reject(err);
//             })
//             .run();
//     });
